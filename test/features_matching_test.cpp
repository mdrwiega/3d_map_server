...
class FeaturesMatchingTest : public ::testing::Test
{
 public:
  ...
  void configure() {
    ...
    cfg_.feature_cloud.keypoints_method = FeatureCloud::KeypointsDetectMethod::Iss3d;
...
TEST_F(FeaturesMatchingTest, Test_fr)
{
  ...
  // Prepare scene feature cloud
  scene_ = std::make_shared<FeatureCloud>(scene_cloud, cfg_.feature_cloud);
  auto start = std::chrono::high_resolution_clock::now();
  scene_->downsampleAndExtractKeypoints();
  auto diff = std::chrono::duration_cast<std::chrono::milliseconds>(
      std::chrono::high_resolution_clock::now() - start);
  std::cout << "From cloud (size: " << scene_->getPointCloud()->size() << ") extracted "
      << scene_->getKeypoints()->size() << " keypoints in: "
      << diff.count() << " ms." << std::endl;
  scene_->computeSurfaceNormals();
  scene_->computeDescriptors();

  // Prepare model feature cloud
  model_ = std::make_shared<FeatureCloud>(model_cloud, cfg_.feature_cloud);
  start = std::chrono::high_resolution_clock::now();
  model_->downsampleAndExtractKeypoints();
  diff = std::chrono::duration_cast<std::chrono::milliseconds>(
      std::chrono::high_resolution_clock::now() - start);
  std::cout << "From cloud (size: " << model_->getPointCloud()->size() << ") extracted "
      << model_->getKeypoints()->size() << " keypoints in: "
      << diff.count() << " ms." << std::endl;
